...
const Masthead = ({ people }: { title: string; people: Person[] }) => {
  ...
  const allRoles = ['All', ...Array.from(new Set(people.flatMap((person) => person.role.split(", "))))].sort();
  const allTeams = ['All', ...Array.from(new Set(people.flatMap((person) => person.team.split(", "))))].sort();
